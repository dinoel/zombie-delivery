...
// Без теней промежуточный буфер не нужен: тот же градиент можно сразу нанести
// на город и на маску ночи. На Canvas 2D это убирает два крупных drawImage на свет.
...
function collectLights(g, camx, camy) {
  ...
  for (const c of g.cars) {
    ...
    // Каждая фара — отдельная деталь: разбитая больше не создаёт ни луча, ни свечения.
    ...
    add(c.x, c.y, 52, RGB_ROOF_RED, .8 * pulse, .34, .5, 0, 0, 0, 1, c);      // отсвет на крыше
  ...
  add(p.x, p.y, lit ? 74 : 52, RGB_WARM, lit ? .5 : .3, .1, lit ? .8 : .6, 0, 0, 0, 9); // круг под ногами
...
function drawLight(g, camx, camy) {
  g.lit = collectLights(g, camx, camy);           // тот же список использует свечение сквозь туман
